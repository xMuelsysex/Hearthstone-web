...
test('public inspection stays on public entities and inspection-only keyboard input is inert', async ({ page }) => {
  ...
  await expect(publicCard).toHaveCSS('opacity', '0')
  const handBox = await publicCard.boundingBox()
  const handPreviewCardBox = await page.locator('[data-inspection-overlay] .hand-hover-card').boundingBox()
  expect(handPreviewCardBox?.width ?? 0).toBeGreaterThan(300)
